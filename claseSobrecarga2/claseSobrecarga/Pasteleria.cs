...
//		}
		...
		{
			string s = string.Format("[Pasteleria Duenio={0}, Direccion={1}, Telefono={2}, Nroproductos={3}]\n", duenio, direccion, telefono, nroproductos);
			for (int i = 0 ;i < nroproductos ;i++) {
				s += "prod " + i + ": " + producto[i,0] + " " + producto[i,1] + " " + producto[i,2] + " " + producto[i,3] + "\n";    
			}
			return s;
		...
		public void agregaProducto (string nom,string cos, string dis,string tip) {
			producto[nroproductos,0] = nom;
			producto[nroproductos,1] = cos;
			producto[nroproductos,2] = dis;
			producto[nroproductos,3] = tip;
			nroproductos++;
		}
